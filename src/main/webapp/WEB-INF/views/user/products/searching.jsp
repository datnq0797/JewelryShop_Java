...
<body>
	...
	<form:form action="/myapp/searching" method="POST" commandName="source">
		<form:input type="text" class="span3" path="text"
		...
		<div class="pagination">
			...
			<c:forEach var="item" begin="1" end="${paginateInfo.totalPage }"
				...
				<c:if test="${(loop.index) == paginateInfo.currentPage }">
					<a href="<c:url value="/searching/${idCategory}/${loop.index }" />"
						class="active">${loop.index }</a>

				</c:if>
				<c:if test="${(loop.index) != paginateInfo.currentPage }">
					<a href="<c:url value="/searching/${idCategory}/${loop.index }" />">${loop.index }</a>
